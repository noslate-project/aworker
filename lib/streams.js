...
const { assert, AssertionError } = load('assert');
...
class ReadableByteStreamController {
  ...
  enqueue(chunk) {
    ...
    if (isArrayBufferDetached(chunk.buffer)) {
      throw new TypeError(
...
class TransformStream {
  constructor(
    ...
    const startPromise = createDeferred();
  ...
  constructor(stream) {
    ...
    if (state === 'writable') {
      if (
        ...
        this[sym.readyPromise] = createDeferred();
      } else {
        this[sym.readyPromise] = { promise: Promise.resolve() };
      }
      this[sym.closedPromise] = createDeferred();
...
function createAlgorithmFromUnderlyingMethod(
  ...
  if (method) {
    ...
    if (algoArgCount === 0) {
      return async () => ReflectApply(method, underlyingObject, extraArgs);
    }
    return async arg => {
      const fullArgs = [ arg, ...extraArgs ];
      return ReflectApply(method, underlyingObject, fullArgs);
    };
...
function invokeOrNoop(
  ...
  return ReflectApply(method, o, args);
}
...
function readableStreamReaderGenericInitialize(
  ...
  if (stream[sym.state] === 'readable') {
    reader[sym.closedPromise] = createDeferred();
...
function readableStreamTee(
  ...
  let branch1;
  let branch2;
  const cancelPromise = createDeferred();
  const pullAlgorithm = () => {
    if (reading) {
      return Promise.resolve();
    }
...
function setUpTransformStreamDefaultControllerFromTransformer(
  ...
  if (transformMethod) {
    ...
    transformAlgorithm = async chunk =>
      ReflectApply(transformMethod, transformer, [ chunk, controller ]);
  }
...
function transformStreamSetBackpressure(
  ...
  stream[sym.backpressureChangePromise] = createDeferred();
...
function writableStreamDefaultWriterWrite(
  ...
  if (stream !== writer[sym.ownerWritableStream]) {
    return Promise.reject(new TypeError('Writer has incorrect WritableStream.'));
  }
  const state = stream[sym.state];
  if (state === 'errored') {
    return Promise.reject(stream[sym.storedError]);
...
function writableStreamUpdateBackpressure(
  ...
  if (writer && backpressure !== stream[sym.backpressure]) {
    if (backpressure) {
      writer[sym.readyPromise] = createDeferred();
    } else {
