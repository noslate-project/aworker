...
const { requiredArguments, applyMixin } = load('utils');
...
const headersDataUnique = Symbol('headers data owned');
...
function copyOnWriteHeaderData(headers) {
  if (headers[headersData][headersDataUnique]) {
    return;
  }
  const data = headers[headersData];
  const dup = new Array(data.length);
  for (let idx = 0; idx < data.length; idx++) {
    dup[idx] = [ data[idx][0], data[idx][1] ];
  }
  headers[headersData] = dup;
  dup[headersDataUnique] = true;
}
...
class Headers {
  constructor(init) {
    ...
    if (isHeaders(init)) {
      this[headersData] = init[headersData];
      this[headersData][headersDataUnique] = false;
    } else {
      this[headersData] = [];
      this[headersData][headersDataUnique] = true;
  ...
  append(name, value) {
    ...
    copyOnWriteHeaderData(this);

    if (this[headersGuard] === 'request') {
...
applyMixin(DomIterableMixin(headersData), Headers);

function isHeaders(value) {
  return typeof value === 'object' && value !== null && headersData in value;
}
