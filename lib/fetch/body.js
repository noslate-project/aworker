...
function bodyToArrayBuffer(bodySource) {
  ...
  if (isTypedArray(bodySource)) {
    // Converts the TypedArray to a non-bigint TypedArray and copy the content
    // into a new ArrayBuffer.
    return new Uint8Array(new Uint8Array(
      bodySource.buffer, bodySource.byteOffset, bodySource.byteLength)
    ).buffer;
  } else if (bodySource instanceof ArrayBuffer) {
    return bodySource;
  } else if (typeof bodySource === 'string') {
...
class Body {
  ...
  constructor(bodySource, meta) {
    validateBodyType(this, bodySource);
    // Preserve UTF-8 BOM in string body.
    if (typeof bodySource === 'string' || bodySource instanceof ReadableStream) {
      this._bodySource = bodySource;
    } else {
      this._bodySource = bodySource != null
        ? bodyToArrayBuffer(bodySource)
        : bodySource;
    }
  ...
  get body() {
    ...
    if (!this._bodySource) {
      ...
      const buf = typeof this._bodySource === 'string'
        ? bodyToArrayBuffer(this._bodySource)
        : this._bodySource;
  ...
  arrayBuffer() {
    ...
    let buf = this._bodySource;
    if (!buf) {
      buf = new ArrayBuffer(0);
    }
    if (typeof this._bodySource === 'string') {
      buf = bodyToArrayBuffer(buf);
    }
...
class Response extends Body {
  constructor(body = null, init) {
    ...
    const extraInit = init[platform_private_options_symbol] || {};
