...
const i18n = loadBinding('i18n');
const {
  CONVERTER_FLAGS_NONE,
  CONVERTER_FLAGS_FATAL,
  CONVERTER_FLAGS_IGNORE_BOM,
  CONVERTER_FLAGS_STREAM,
} = i18n;

const empty = new Uint8Array(0);

function validateEncoder(obj) {
  if (obj == null || obj[kEncoder] !== true) { throw new Error('Invalid TextEncoder'); }
}

function validateDecoder(obj) {
  if (obj == null || obj[kDecoder] !== true) { throw new Error('Invalid TextDecoder'); }
}

function validateArgument(prop, expected, propName, expectedName) {
  if (typeof prop !== expected) {
    throw new Error(`Type error, ${prop} should be ${expectedName} not ${propName}`);
  }
}
...
class TextEncoder {
  ...
  encode(input = '') {
    validateEncoder(this);
    return i18n.encodeUtf8(`${input}`);
  }

  encodeInto(src, dest) {
    validateEncoder(this);
    if (!dest || !isUint8Array(dest)) {
      throw new Error('dest should be Uint8Array');
    }
    const result = new Uint32Array(2);
    i18n.encodeIntoUtf8(src, dest, result);
...
class TextDecoder {
  constructor(encoding = 'utf-8', options = {}) {
    ...
    let flags = CONVERTER_FLAGS_NONE;
    if (options != null) {
      flags |= options.fatal ? CONVERTER_FLAGS_FATAL : CONVERTER_FLAGS_NONE;
      flags |= options.ignoreBOM ? CONVERTER_FLAGS_IGNORE_BOM : CONVERTER_FLAGS_NONE;
    }

    const handle = new i18n.Converter(enc, flags);
    if (handle === undefined) {
      throw new Error('encoding is not supported');
    }
  ...
  decode(input = empty, options = {}) {
    ...
    if (isAnyArrayBuffer(input)) {
      ...
      throw new TypeError('input must be either ArrayBuffer or ArrayBufferView');
    }
    ...
    let flags = CONVERTER_FLAGS_NONE;
    if (options != null) {
      flags |= options.stream ? CONVERTER_FLAGS_STREAM : CONVERTER_FLAGS_NONE;
    }
    const handle = this[kHandle];
    return handle.decode(input, flags);
  }
}
