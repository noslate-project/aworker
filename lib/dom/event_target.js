...
/** @type {WeakMap<EventTarget, Map<string, EventTargetSafeListeners>>} */
const safeListenersMap = new WeakMap();
const globalMap = new Map();
function EventTarget() {
  // TODO(chengzhong.wcz): remove the dependencies on WeakMap for globalThis.
  let map;
  if (this === globalThis) {
    map = globalMap;
  } else {
...
function getSafeListenersForTarget(target, type) {
  const listenerMap = safeListenersMap.get(target);
  if (listenerMap == null) {
...
function getAttributeListener(target, type) {
  const listeners = safeListenersMap.get(target);
  if (listeners == null) {
    throw new TypeError('');
  }
