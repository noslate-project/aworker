...
class NoslatedClient : public NoslatedService {
 public:
  ...
  explicit NoslatedClient(shared_ptr<EventLoop> loop) {
...
void StressTest(std::shared_ptr<uv::NoslatedClient> client, uint64_t* called) {
  ...
  client->Request(
      client->NewControllerWithTimeout(1000),
      std::move(req),
...
TEST(StressTest, UvServerUvClient) {
  ...
  std::shared_ptr<uv::NoslatedClient> client =
      make_shared<uv::NoslatedClient>(make_shared<UvLoop>(&client_loop));
  UvSocketHolder::Connect(
      make_shared<UvLoop>(&client_loop),
      server_path,
      client->delegate(),
      [client, called = &called](UvSocketHolder::Pointer socket) {
