...
std::string DiagnosticFilename::MakeFilename(const char* prefix,
                                             const char* ext) {
...
static void ReportPipeEndpoints(uv_handle_t* h, JSONWriter* writer) {
  uv_any_handle* handle = reinterpret_cast<uv_any_handle*>(h);
  ResizableBuffer buffer(0);
  size_t buffer_size = 0;
  int rc = -1;

  // First call to get required buffer size.
  rc = uv_pipe_getsockname(
      &handle->pipe, static_cast<char*>(buffer.buffer()), &buffer_size);
  if (rc == UV_ENOBUFS) {
  ...
  if (rc == 0 && buffer_size != 0 && buffer.buffer() != nullptr) {
    writer->json_keyvalue("localEndpoint", static_cast<char*>(buffer.buffer()));
  } else {
  ...
  if (rc == 0 && buffer_size != 0 && buffer.buffer() != nullptr) {
    writer->json_keyvalue("remoteEndpoint",
...
static void ReportPath(uv_handle_t* h, JSONWriter* writer) {
  ...
  switch (h->type) {
    case UV_FS_EVENT:
      rc = uv_fs_event_getpath(
          &(handle->fs_event), static_cast<char*>(buffer.buffer()), &size);
      break;
    default:
      break;
  }
  if (rc == UV_ENOBUFS) {
    buffer.Realloc(size + 1);
    switch (h->type) {
      case UV_FS_EVENT:
        rc = uv_fs_event_getpath(
            &(handle->fs_event), static_cast<char*>(buffer.buffer()), &size);
    ...
    if (rc == 0) {
      ...
      static_cast<char*>(buffer.buffer())[size] = '\0';
      writer->json_keyvalue("filename", static_cast<char*>(buffer.buffer()));
      wrote_filename = true;
    }
  }
