...
std::unique_ptr<v8_inspector::StringBuffer> Utf8ToStringBuffer(
...
class MainThreadInterface
 ...
 public:
  explicit MainThreadInterface(InspectorAgent* agent);
  ~MainThreadInterface();
  ...
  InspectorAgent* inspector_agent() { return agent_; }
 ...
 private:
  ...
  std::mutex requests_lock_;  // requests_ live across threads
