...
#define TRACE_STR_COPY(str) aworker::tracing::TraceStringWithCopy(str)

// By default, uint64 ID argument values are not mangled with the Process ID in
// TRACE_EVENT_ASYNC macros. Use this macro to force Process ID mangling.
#define TRACE_ID_MANGLE(id) aworker::tracing::TraceID::ForceMangle(id)

// By default, pointers are mangled with the Process ID in TRACE_EVENT_ASYNC
// macros. Use this macro to prevent Process ID mangling.
#define TRACE_ID_DONT_MANGLE(id) aworker::tracing::TraceID::DontMangle(id)
...
#define TRACE_EVENT_API_GET_CATEGORY_GROUP_ENABLED                             \
  aworker::tracing::TraceEventHelper::GetCategoryGroupEnabled
...
//                    unsigned int flags)
#define TRACE_EVENT_API_ADD_TRACE_EVENT aworker::tracing::AddTraceEventImpl
...
#define TRACE_EVENT_API_ADD_TRACE_EVENT_WITH_TIMESTAMP                         \
  aworker::tracing::AddTraceEventWithTimestampImpl
...
#define TRACE_EVENT_API_UPDATE_TRACE_EVENT_DURATION                            \
  if (auto controller =                                                        \
          aworker::tracing::TraceEventHelper::GetTracingController())          \
...
#define TRACE_EVENT_API_ADD_METADATA_EVENT aworker::tracing::AddMetadataEvent
...
#define INTERNAL_TRACE_EVENT_ADD(phase, category_group, name, flags, ...)      \
  do {                                                                         \
    ...
    if (INTERNAL_TRACE_EVENT_CATEGORY_GROUP_ENABLED_FOR_RECORDING_MODE()) {    \
      aworker::tracing::AddTraceEvent(                                         \
          phase,                                                               \
          INTERNAL_TRACE_EVENT_UID(category_group_enabled),                    \
          name,                                                                \
          aworker::tracing::kGlobalScope,                                      \
          aworker::tracing::kNoId,                                             \
          aworker::tracing::kNoId,                                             \
          flags,                                                               \
          ##__VA_ARGS__);                                                      \
    }                                                                          \
...
#define INTERNAL_TRACE_EVENT_ADD_SCOPED(category_group, name, ...)             \
  INTERNAL_TRACE_EVENT_GET_CATEGORY_INFO(category_group);                      \
  aworker::tracing::ScopedTracer INTERNAL_TRACE_EVENT_UID(tracer);             \
  if (INTERNAL_TRACE_EVENT_CATEGORY_GROUP_ENABLED_FOR_RECORDING_MODE()) {      \
    uint64_t h = aworker::tracing::AddTraceEvent(                              \
...
#define INTERNAL_TRACE_EVENT_ADD_SCOPED_WITH_FLOW(                             \
  ...
  if (INTERNAL_TRACE_EVENT_CATEGORY_GROUP_ENABLED_FOR_RECORDING_MODE()) {      \
    ...
    aworker::tracing::TraceID trace_event_bind_id(bind_id,                     \
                                                  &trace_event_flags);         \
    uint64_t h = aworker::tracing::AddTraceEvent(                              \
        TRACE_EVENT_PHASE_COMPLETE,                                            \
        INTERNAL_TRACE_EVENT_UID(category_group_enabled),                      \
...
#define INTERNAL_TRACE_EVENT_ADD_WITH_ID(                                      \
  ...
  do {                                                                         \
    ...
    if (INTERNAL_TRACE_EVENT_CATEGORY_GROUP_ENABLED_FOR_RECORDING_MODE()) {    \
      ...
      aworker::tracing::TraceID trace_event_trace_id(id, &trace_event_flags);  \
      aworker::tracing::AddTraceEvent(                                         \
          phase,                                                               \
          INTERNAL_TRACE_EVENT_UID(category_group_enabled),                    \
          name,                                                                \
...
#define INTERNAL_TRACE_EVENT_ADD_WITH_TIMESTAMP(                               \
  ...
  do {                                                                         \
    INTERNAL_TRACE_EVENT_GET_CATEGORY_INFO(category_group);                    \
    if (INTERNAL_TRACE_EVENT_CATEGORY_GROUP_ENABLED_FOR_RECORDING_MODE()) {    \
      aworker::tracing::AddTraceEventWithTimestamp(                            \
          phase,                                                               \
          INTERNAL_TRACE_EVENT_UID(category_group_enabled),                    \
          name,                                                                \
          aworker::tracing::kGlobalScope,                                      \
...
class TraceEventHelper {
 public:
  ...
  static TraceAgent* GetTraceAgent();
  static void SetTraceAgent(TraceAgent* agent);
...
#include "tracing/trace_internal.h"
